...
fn coordinates_steps(path: Vec<&str>) -> Vec<[i64; 3]> {
...
fn intersects(positions_one: Vec<[i64; 2]>, positions_two: Vec<[i64; 2]>) -> Vec<u64> {
    let mut intersects = Vec::new();

    for i in positions_one{
        for _j in &positions_two{
            if i[0] == _j[0] && i[1] == _j[1] {
                let x: u64 = abs(i[0]);
                let y: u64 = abs(i[1]);
                let man_hat = x + y;
                intersects.push(man_hat);
            }
        }
    }
    intersects.sort();
...
fn intersects_steps(positions_one: Vec<[i64; 3]>, positions_two: Vec<[i64; 3]>) -> Vec<u64> {
    let mut intersects = Vec::new();

    for i in positions_one{
        for j in &positions_two{
            if i[0] == j[0] && i[1] == j[1] {
                let x: u64 = i[2] as u64;
                let y: u64 = j[2] as u64;
                let combined_steps = x + y;
                intersects.push(combined_steps);
            }
        }
    }
    intersects.sort();
